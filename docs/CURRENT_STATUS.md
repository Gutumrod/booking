> **Sync note (2026-09-14, Commander):** `main` was frozen at `3aee2a5` (29 ส.ค., docs-only)
> while all real BK-A/BK-SR/R0–R6 work happened on feature branches. This file is copied
> verbatim from `feature/bk01-real-shop-hardening-r4:docs/CURRENT_STATUS.md` (branch HEAD
> `54b10a6`, last commit 2026-09-11 22:03) so `main` reflects the real picture. **This is the
> feature branch's own self-reported status — Commander has not independently re-verified the
> underlying evidence per the Independent Verification policy.** No application code or
> migration was merged to `main` in this sync; code/DB state on `main` is still pre-BK-A.
> Nothing has moved on any branch since 2026-09-11 (checked via `git fetch` on 2026-09-14).
